...
class SCNavigationMenu extends ReduxMixin(Localized(PolymerElement)) {
  static get template() {
    return html`
    ${scrollbarStyle}
    <style>
    ...
    <iron-ajax id="menu_ajax" url="[[_getMenuUrl()]]" handle-as="json" loading="{{loading}}" last-error="{{mainMenuError}}" last-response="{{mainMenuData}}" on-response="_generateMainMenu">
    </iron-ajax>

    <iron-ajax id="submenu_ajax" url="[[_getSubMenuUrl()]]" handle-as="json" loading="{{loading}}" last-error="{{childMenuError}}" last-response="{{subMenuData}}" on-response="_createChildMenu">
    </iron-ajax>
