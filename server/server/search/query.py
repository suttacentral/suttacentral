...
def get_available_indexes(indexes, _cache=util.TimedCache(lifetime=30)):
    ...
    for index in indexes:
        try:
            if es.cluster.health(index, timeout='100ms')['status'] in {
                'green',
                'yellow',
            }:
    ...
    body = {
        ...
        "query": {
            "function_score": {
                "query": inner_query,
                "functions": [
                    {"weight": "1.5", "filter": {"term": {"lang": language}}},
                    {
                        "field_value_factor": {
                            "field": "boost",
                            "factor": 1.0,
                    ...
                    {"weight": "0.25", "filter": {"type": {"value": "definition"}}},
                    {
                        "weight": "2",
                        "filter": {"term": {"uid": query.replace(' ', '').lower()}},
                    },
                    {"weight": "1.0", "filter": {"term": {"is_root": True}}},
                ],
                "score_mode": "multiply",
            }
        },
