...
class SecondMigration(Migration):
    migration_id = 'add_indexes_020'
    tasks = ['add_indexes']

    def add_indexes(self):
        db = get_db()

        po_strings = db['po_strings']
        po_strings.add_hash_index(fields=['uid'], unique=False)
        po_strings.add_hash_index(fields=['lang'], unique=False)
