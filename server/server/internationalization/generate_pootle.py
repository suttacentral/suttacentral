...
def process_blurbs(db):
    ...
    for doc in tqdm(db.aql.execute(BLURBS_QUERY)):
        ...
        for uid in sorted(entries.keys(), key=uid_sort_key):
            blurb = entries[uid]
            pot.append(polib.POEntry(msgid=blurb, msgctxt=uid))
...
def process_menu(db):
    ...
    for lang, po_file in tqdm(po_files.items()):
        ...
        for name in root_names:
            ...
            entry = polib.POEntry(
                msgid=name['name'], msgstr=msgstr, msgctxt=name['uid']
            )
...
def process_currencies(db):
    ...
    for name in tqdm(currency_names_collection.find({'lang': 'en'})):
        entry = polib.POEntry(
            msgid=name['name'], msgstr=name['name'], msgctxt=name['symbol']
        )
        po.append(entry)
        pot.append(polib.POEntry(msgid=name['name'], msgctxt=name['symbol']))

    (GENERATED_PO_FILES_DIR / 'server_currency_names').mkdir(
