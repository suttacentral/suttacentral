...
def generate_sitemap(db):
    ...
    for text in sorted(html_texts, key=sort_key):
    ...
    for uid, texts_by_lang in sorted(texts_by_uid.items()):
        ...
        if 'en' in texts_by_lang:
            entries.append(
                url_template.render(
                    text=texts_by_lang['en'],
                    other_texts=non_en_texts,
                    create_url=create_url,
                )
            )
        else:
            entries.append(
                alt_url_template.render(
                    urls=[create_url(text) for text in non_en_texts]
                )
            )
