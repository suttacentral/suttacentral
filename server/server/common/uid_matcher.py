...
decompose_rex = regex.compile(
    r'(?<prefix>[a-z]++.*?)(?<num_start>\d+)(?:-\1?(?<num_end>\d+))?$'
)

# this regex deals with the rather insane case like an1.188-197-an1.219-234
decompose_x_rex = regex.compile(
    r'(?<prefix>[a-z]++.*?)(?<num_start>\d+)(?:-\d+-\1\d+-(?<num_end>\d+))$'
)
    ...
    def decompose(self, uid):
        ...
        if m:
            ...
            if duplicate_string_rex.match(prefix):
                logging.error(
                    f'Prefix {prefix} determined for uid {uid}, malformed uid suspected'
                )
            if prefix.endswith('-'):
                ...
                if num_end < num_start:
                    logging.error(
                        f'Uid {uid} appears to contain a range, but the {num_end} < {num_start}'
                    )
